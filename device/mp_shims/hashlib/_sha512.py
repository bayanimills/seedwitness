...
class sha512(sha):
    ...
    def _transform(self):
        # SHA-512 only needs the previous 16 schedule words to calculate the
        # next one.  Keeping all 80 is convenient but expensive on an ESP32:
        # they are heap-allocated Python bigints, and growing the list itself
        # eventually asks for a 512-byte contiguous block as the schedule
        # grows.  That exact allocation is the field failure recorded as
        # ``MemoryError _sha512.py:55`` after the derivation modules loaded.
        # A 16-word ring holds the same recurrence with a fixed-size list and
        # removes both the list growth and 64 live bigints per transform.
        W = [0] * 16
        ...
        def RND(a, b, c, d, e, f, g, h, i, ki):
            ...
            if i >= 16:
                W[j] = (
                    Gamma1(W[(j - 2) & 15]) + W[(j - 7) & 15]
                    + Gamma0(W[(j - 15) & 15]) + W[j]
                ) & 0xFFFFFFFFFFFFFFFF
            t0 = (h + Sigma1(e) + Ch(e, f, g) + ki + W[j]) & 0xFFFFFFFFFFFFFFFF

...
Vendored from rdagger/micropython-ili9341 (MIT license), fetched
2026-08-01, with ONE local modification: clear() reuses a cached line
buffer instead of allocating a fresh one per call -- see clear()'s
docstring for the measured OOMs that forced this. Everything else is
unmodified. draw_text8x8() (this repo's chosen text-rendering method) uses
MicroPython's built-in framebuf.FrameBuffer.text() 8x8 font -- no external
font file/asset needed, keeping this a single self-contained driver file.
See device/seedwitness/drivers/cyd.py for CYD-specific pin wiring.
"""
...
class Display(object):
    ...
    def clear(self, color=0, hlines=8):
        ...
        # LOCAL MODIFICATION (seedwitness). The vendored code allocated the
        # line buffer fresh on every call: w * 2 * hlines contiguous bytes,
        # 3840 at 240x8. A full-screen clear opens every redraw in this app,
        # so that demand recurred on every frame -- and on this board's
        # ~20KB post-import heap, whose largest contiguous run measures
        # 1-5KB, it is the exact allocation every observed MemoryError died
        # on: the boot-time first draw, mid-session redraws under a held
        # touch, and the crash screen itself (which turned a recoverable
        # error into a hard reset; see diag.txt "crash MemoryError" +
        # "boot HARD_RESET" from the field). Allocate once -- the first
        # call comes from __init__, the healthiest the heap ever is -- and
        # refill in place only when the colour changes; the fill logic and
        # the buffer now live in _pattern(), because fill_hrect/fill_vrect,
        # draw_hline/draw_vline and draw_text8x8 all had the same disease
        # and now share the same cure.
        need = w * 2 * hlines
        self._pattern(color, need)
        line = self._clear_line
        if len(line) != need:
            # only when a caller passed a non-default hlines; the normal
            # buffer is exactly one clear line and is handed over whole so
            # the cached object itself reaches block()
            line = memoryview(line)[:need]
        for y in range(0, h, hlines):
            self.block(0, y, w - 1, y + hlines - 1, line)

    def _pattern(self, color, size):
        """LOCAL MODIFICATION (seedwitness). `size` bytes of `color` as
        big-endian RGB565, served from the one cached line buffer clear()
        introduced.

        Every chunked writer in this driver had clear()'s original disease:
        a fresh KB-scale bytearray per call (fill_hrect/fill_vrect up to
        2048 bytes per rectangle, draw_text8x8 up to 3840 per string), many
        times per frame, on a heap whose largest free run measures 1-2KB
        mid-session. Measured on the board, the hold-to-confirm sweep died
        exactly there: fill_hrect's 1681-byte chunk was the allocation the
        Generate Seed MemoryError failed on. One shared buffer, refilled in
        place only when the colour changes (the doubling below is memoryview
        slice copies -- no allocation), removes the draw path's transient
        KB-scale demand entirely. draw_text8x8 rasterises arbitrary pixels
        into the same buffer and marks the pattern dirty (_clear_color =
        None) so the next patterned caller refills.

        Returns a memoryview of exactly `size` bytes so block() writes just
        that window."""
        line = getattr(self, "_clear_line", None)
        if line is None or len(line) < size:
            # width * 16 covers every sharer: a clear line is width*2*8 and
            # a full-width 8x8 text strip is width*8*2 rows of 16 bytes --
            # both 3840 at 240 wide. Allocated once, from __init__'s first
            # clear(), the healthiest the heap ever is.
            line = bytearray(max(size, self.width * 16))
            self._clear_line = line
            self._clear_color = None
        if self._clear_color != color:
    ...
    def draw_hline(self, x, y, w, color):
        ...
        if self.is_off_grid(x, y, x + w - 1, y):
            return
        # LOCAL MODIFICATION (seedwitness): shared cached pattern buffer
        # instead of a per-call allocation; w <= width, well under the
        # buffer. See _pattern().
        self.block(x, y, x + w - 1, y, self._pattern(color, w * 2))
    ...
    def draw_text8x8(self, x, y, text, color,  background=0,
        ...
        if self.is_off_grid(x, y, x + 7, y + 7):
            return
        # LOCAL MODIFICATION (seedwitness). The vendored code allocated a
        # fresh w*16-byte buffer per string -- 3840 bytes for a full-width
        # line, on every secondary-text draw of every frame, which is the
        # same per-frame KB-scale churn clear() was cured of. Rasterise
        # into the shared cached buffer instead when it fits (it always
        # does for on-screen text: w <= width). The buffer then holds
        # arbitrary glyph pixels rather than a colour pattern, so mark the
        # pattern dirty for the next _pattern() caller. The background fill
        # is now unconditional: a reused buffer holds stale bytes where the
        # vendored fresh allocation was implicitly zeroed.
        need = w * 16
        line = getattr(self, "_clear_line", None)
        if line is not None and len(line) >= need:
            buf = memoryview(line)[:need]
            self._clear_color = None
        else:
            buf = bytearray(need)
    ...
    def draw_vline(self, x, y, h, color):
        ...
        if self.is_off_grid(x, y, x, y + h - 1):
            return
        # LOCAL MODIFICATION (seedwitness): shared cached pattern buffer
        # instead of a per-call allocation; h <= height = 320, 640 bytes,
        # well under the buffer. See _pattern().
        self.block(x, y, x, y + h - 1, self._pattern(color, h * 2))
    ...
    def fill_hrect(self, x, y, w, h, color):
        ...
        chunk_y = y
        # LOCAL MODIFICATION (seedwitness): chunks come from the shared
        # cached pattern buffer instead of a fresh bytearray per call; the
        # chunking maths bounds every request at 2048 bytes, under the
        # buffer's 3840. See _pattern() for why.
        if chunk_count:
            buf = self._pattern(color, chunk_size * 2)
            for c in range(0, chunk_count):
                self.block(x, chunk_y,
        ...
        if remainder:
            self.block(x, chunk_y,
                       ...
                       self._pattern(color, remainder * w * 2))
    ...
    def fill_vrect(self, x, y, w, h, color):
        ...
        chunk_x = x
        # LOCAL MODIFICATION (seedwitness): shared cached pattern buffer,
        # same as fill_hrect. See _pattern().
        if chunk_count:
            buf = self._pattern(color, chunk_size * 2)
            for c in range(0, chunk_count):
        ...
        if remainder:
            self.block(chunk_x, y,
                       ...
                       self._pattern(color, remainder * h * 2))
